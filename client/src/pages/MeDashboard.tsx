...
export default function MeDashboard() {
  ...
  const artists = useApi<RankedListResponse>(`/api/me/stats/top-artists?period=${period}`, [period], { pollMs: TOP_LIST_POLL_MS });
  const albums = useApi<RankedListResponse>(`/api/me/stats/top-albums?period=${period}`, [period], { pollMs: TOP_LIST_POLL_MS });
  const tracks = useApi<RankedListResponse>(`/api/me/stats/top-tracks?period=${period}`, [period], { pollMs: TOP_LIST_POLL_MS });
  const recent = useApi<ScrobblesResponse>(`/api/me/scrobbles?limit=25`, [], { pollMs: SCROBBLE_POLL_MS });
  const heatmap = useApi<HeatmapResponse>(`/api/me/stats/heatmap?year=${year}`, [year], { pollMs: 5 * 60_000 });
  const summary = useApi<SummaryDto>(`/api/me/stats/summary`, [], { pollMs: 5 * 60_000 });
  const timeOfDay = useApi<TimeOfDayResponse>(`/api/me/stats/time-of-day`, [], { pollMs: 5 * 60_000 });
      ...
      <section>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          <div>
            ...
            <HeroCard item={artistItems[0]} kind="artist" emptyLabel="No artist data yet" />
            <div className="mt-3">
          ...
          <div>
            ...
            <HeroCard item={albumItems[0]} kind="album" emptyLabel="No album data yet" />
            <div className="mt-3">
          ...
          <div>
            ...
            <StatList items={trackItems.slice(0, 8)} startRank={1} kind="track" />
          </div>
        </div>
      </section>
