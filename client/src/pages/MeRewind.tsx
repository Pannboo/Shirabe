...
function fmtLongDate(iso: string): string {
  // SQLite returned a YYYY-MM-DD string — render as "12 March".
...
export default function MeRewind() {
  const [year, setYear] = useState<number>(new Date().getFullYear());
  const { data, loading } = useApi<RewindResponse>(`/api/me/stats/rewind/${year}`, [year]);
    ...
    <div className="space-y-14">
      ...
      {data && (
        <>
          ...
          <section>
            <SectionTitle>Your year, by the numbers</SectionTitle>
          ...
          {(data.new_artists_discovered > 0 || data.new_albums_discovered > 0) && (
            <section>
              <SectionTitle>What you discovered</SectionTitle>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-x-10 gap-y-6">
                <Storyline
                  label="New artists"
...
function BookendCard({ label, item }: { label: string; item: RewindHighlight }) {
  const albumHref = item.album
    ? `/me/album/${encodeURIComponent(item.artist)}/${encodeURIComponent(item.album)}`
    : null;
  const trackHref = `/me/track/${encodeURIComponent(item.artist)}/${encodeURIComponent(item.track)}`;
  return (
